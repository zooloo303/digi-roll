...
Status: **planned, nothing built.** Branch `pattern-generator`.
Decided with Neil 2026-08-09; the four scope questions and their answers are at
the bottom.
...
Nothing here needs new hardware verification to be *safe* — the generator emits
the same pattern state a hand-drawn pattern emits, over write paths already
verified on both boxes. The smoke test is musical rather than protocol work, and
belongs at the end of phase 5:

10. Generate a DnB arrangement, send bass/chords/lead to three tracks, and check
    on the box that the notes, per-trig conditions and p-lock lanes are what the
    roll drew, that playback sounds like the genre, and that **the destination
