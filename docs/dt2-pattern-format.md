...
| 88762 | 6 | pattern settings: **88764 is SWING**, stored as the offset from straight (`0` = 50%, `30` = 80%) — the one edited pattern in the fixtures holds `5`, i.e. 55%, and the other 127 hold `0` [F]. Mapped on the DN2 at the sibling offset ([V4] there, where the write is hardware-verified too) and confirmed here across both fixtures — but **no swing has been written to a DT2**, so that half rests on the sibling layout. Remaining 5 bytes unknown |
